        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f"♨️[{get_size(file.file_size)}]♨️{file.file_name}", url=f"https://telegram.dog/Testv105robot?start={file.file_id}"
                ),
            ]
            for file in files
        ]
...
async def auto_filter(client, msg, spoll=False):
    ...
    if settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"[{get_size(file.file_size)}] {file.file_name}", url=f"https://telegram.dog/Testv105robot?start={file.file_id}"
                ),
            ]
            for file in files
        ]
